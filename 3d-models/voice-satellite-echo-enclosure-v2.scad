...
module top_cap_assembly() {
    difference() {
        ...
        // Hollow interior - THREE STAGE DESIGN
        // Stage 1: Lower hollow section (z = 2.5mm to 13mm, leaves 2mm wall for speaker grill at z=15mm)
        translate([0, 0, wall_thickness])
            cylinder(h = 13 - wall_thickness, d = enclosure_diameter - wall_thickness * 2, $fn = 100);

        // Stage 2: Middle hollow section (z = 13mm to 17mm, narrower to leave material for speaker grill ring)
        translate([0, 0, 13])
            cylinder(h = 4, d = speaker_diameter - 10, $fn = 80);

        // Stage 3: Upper hollow section (z = 17mm to 12mm, narrower for LED ring ledge)
        // Note: This overlaps with Stage 2 intentionally to create the right cavity shape
        translate([0, 0, 10])
            cylinder(h = 7, d = led_ring_inner_diameter - 4, $fn = 80);
        ...
        // Speaker grill holes (cut through wall at 3/4 height = 15mm)
        translate([0, 0, 15 - 1])
